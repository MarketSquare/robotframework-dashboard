...
The data is designed to fill every graph: persistent failures, flaky tests, tests broken/fixed at a given run, an outage run per project (`WebshopUI` run 7 on 2026-08-28, `WebshopAPI` run 5), one all-green run (`WebshopAPI` run 4, 2026-08-25) and one pass+skip-only run (`WebshopAPI` run 6, 2026-09-02), `TRY/EXCEPT` exceptions, feature-flag skips, slow outliers and duration trends. `tests/robot/resources/generator/libraries/profiles.py` says which test does what.
