...
All tests are **Robot Framework acceptance tests** run with **pabot**. There are no pytest or unittest files. Tests live in `tests/e2e/testsuites/`.
...
# Linux / macOS
pabot --pabotlib --testlevelsplit --artifacts png,jpg --artifactsinsubfolders --processes 2 -d results tests/e2e/testsuites/*.robot
```
...
|---|---|---|
| `00_cli.robot` | Every CLI flag (short + long form) | Runs `robotdashboard` as a subprocess, checks stdout/files against `tests/e2e/resources/cli_output/` |
| `01_database.robot` | SQLite table contents after parsing | Queries the DB via DatabaseLibrary, compares rows against `tests/e2e/resources/database_output/` |
...
`tests/e2e/testsuites/__init__.robot` is the **suite init** — it detects OS at suite setup (stored as global variable), sets a 60-second global test timeout, and runs `Remove Index` + `Move All Screenshots` as a single teardown (`Run Teardown Only Once`).
...
- `tests/e2e/resources/keywords/general-keywords.resource` provides `Get Dashboard Index`
- Uses `Acquire Lock` (pabot lock) to atomically read/write `tests/e2e/resources/index.txt`
...
- Compares output against expected files in `tests/e2e/resources/cli_output/`
...
- Result strings are normalized (path separators, timezone offsets) via regex before comparison against `tests/e2e/resources/database_output/` reference files
...
- Take screenshots and compare with reference images in `tests/e2e/resources/dashboard_output/`
...
| File | Contents |
|---|---|
| `tests/e2e/resources/keywords/general-keywords.resource` | `Get Dashboard Index`, `Generate Dashboard`, `Remove Database And Dashboard With Index` |
| `tests/e2e/resources/keywords/database-keywords.resource` | DB connection helpers, normalization, row comparison |
| `tests/e2e/resources/keywords/dashboard-keywords.resource` | Browser lifecycle, filter helpers (`Set Run Filter`, `Set Date Filter`, `Set Amount Filter`), screenshot comparison, `Change Settings` |
| `tests/e2e/resources/outputs/` | Input `output.xml` files used as test fixtures |
| `tests/e2e/resources/cli_output/` | Expected CLI output reference files |
| `tests/e2e/resources/database_output/` | Expected DB row reference files |
| `tests/e2e/resources/dashboard_output/` | Reference screenshots for visual comparison |

---

## Adding a New Test

1. **CLI test**: add two test cases in `00_cli.robot` (short + long form), add expected output file to `tests/e2e/resources/cli_output/`
2. **Database test**: add test case in `01_database.robot`, add reference row file to `tests/e2e/resources/database_output/`
3. **Browser test**: add test case in the relevant suite (`02–07`), capture a reference screenshot and add to `tests/e2e/resources/dashboard_output/`
