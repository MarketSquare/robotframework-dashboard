...
- 🔎 **Global Filters** - Filter runs by name, tags, date, metadata, or quantity (applied per project). Drag across the runs-over-time histogram to zoom into a period, the way Grafana and Kibana do. Every filter option shows how many runs it still matches, and options that match none are greyed out.  
